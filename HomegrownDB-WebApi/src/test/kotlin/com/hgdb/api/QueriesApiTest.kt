...
import com.hgdb.core.config.DatabaseProvider
...
class QueriesApiTest {
    ...
    fun `test readAll queries`() {
        ...
        val query = Query(0, "my query", "SELECT we FROM we")
        queriesApi.create(Request(Method.POST, "/queries").with(
        ...
        val returnedQueries = Json.decodeFromString<Array<Query>>(response.bodyString())
